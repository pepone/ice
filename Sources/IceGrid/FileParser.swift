...
public extension FileParserPrx {
    ...
    func parseAsync(xmlFile iceP_xmlFile: Swift.String, adminProxy iceP_adminProxy: AdminPrx?, context: Ice.Context? = nil, sentOn: Dispatch.DispatchQueue? = PromiseKit.conf.Q.return, sentFlags: Dispatch.DispatchWorkItemFlags? = nil, sent: ((Swift.Bool) -> Swift.Void)? = nil) -> PromiseKit.Promise<ApplicationDescriptor> {
...
public struct FileParserDisp: Ice.Disp {
    ...
    public func dispatch(request: Ice.Request, current: Ice.Current) throws {
        ...
        case "ice_id":
            try (servant as? Object ?? FileParserDisp.defaultObject)._iceD_ice_id(incoming: request, current: current)
        case "ice_ids":
            try (servant as? Object ?? FileParserDisp.defaultObject)._iceD_ice_ids(incoming: request, current: current)
        case "ice_isA":
            try (servant as? Object ?? FileParserDisp.defaultObject)._iceD_ice_isA(incoming: request, current: current)
        case "ice_ping":
            try (servant as? Object ?? FileParserDisp.defaultObject)._iceD_ice_ping(incoming: request, current: current)
        case "parse":
            try servant._iceD_parse(incoming: request, current: current)
        default:
...
public extension FileParser {
    func _iceD_parse(incoming inS: Ice.Incoming, current: Ice.Current) throws {
