...
bzip2_url = (
    f"https://github.com/zeroc-ice/bzip2/archive/refs/tags/v{bzip2_version}.tar.gz"
)
...
if sys.platform == "win32":
    ice_cpp_sources.append(
        f"../cpp/include/generated/cpp98/{platform}/{configuration}/Ice"
    )
    ice_cpp_sources.append(
        f"../cpp/include/generated/cpp98/{platform}/{configuration}/IceSSL"
    )
    ice_cpp_sources.append(
        f"../cpp/include/generated/cpp98/{platform}/{configuration}/IceDiscovery"
    )
    ice_cpp_sources.append(
        f"../cpp/include/generated/cpp98/{platform}/{configuration}/IceLocatorDiscovery"
    )
...
if sys.platform == "darwin":
    ...
    cpp_extra_compile_args = ["-std=c++14"]
    libraries = []
    extra_link_args = ["-framework", "Security", "-framework", "CoreFoundation"]
elif sys.platform == "win32":
    define_macros.extend(
        [("WIN32_LEAN_AND_MEAN", None), ("_WIN32_WINNT", "0x0601"), ("BZ_EXPORT", None)]
    )
    extra_compile_args = ["/std:c++14", "/EHsc", "/Zi"]
    ...
    def build_extension(self, ext: Extension):
        ...
        def _compile(
            ...
            original_compile(
                ...
                ext,
                cc_args,
                cpp_extra_compile_args + extra_postargs
                if src.endswith(".cpp")
                else extra_postargs,
                pp_opts,
            )
...
class CustomSdistCommand(_sdist):
    def include_file(self, filename: str):
        ...
        if pathlib.Path(filename).suffix.lower() not in [
            ".c",
            ".cpp",
            ".h",
            ".ice",
    ...
    def pre_build(self):
        ...
        if sys.platform == "win32":
            ...
            os.system(
                f"MSBuild /m ../cpp/msbuild/ice.proj {msbuild_args} /t:NuGetRestore"
            )
            os.system(
                f"MSBuild /m ../cpp/msbuild/ice.sln {msbuild_args} /t:c++98\\slice2cpp;c++98\\slice2py"
            )
            ...
            for project in ["Ice", "IceSSL", "IceDiscovery", "IceLocatorDiscovery"]:
                project_path = (
                    f"../cpp/src/{project}/msbuild/{project}/{project}.vcxproj"
                )
            ...
            os.system(
                f"make OPTIMIZE=yes -C {cpp_source_dir} {cpp_targets} {cpp_source_dir}"
            )
                ...
                for file in files:
                    if self.include_file(os.path.join(root, file)):
                        if sys.platform == "win32":
                            ...
                            if (
                                "generated" in root
                                and os.path.join(platform, configuration) not in root
                            ):
                                continue
                            elif (
                                "msbuild" in root
                            ...
                            for name in [
                                ...
                                if root.startswith(
                                    ...
                                    if file.endswith(".h"):
                                        ...
                                        relative_path = os.path.join(
                                            "cpp", "src", name, "generated", file
                                        )
                                    break

                        target_file = os.path.join(
                            script_directory, "dist/ice", relative_path
                        )
        ...
        for root, dirs, files in os.walk("modules/IcePy"):
            for file in files:
                if file.endswith(".cpp") or file.endswith(".h"):
                    ...
                    relative_path = os.path.relpath(source_file, ".")
                    target_file = os.path.join(
                        script_directory, "dist/ice/python", relative_path
                    )
                    target_dir = os.path.dirname(target_file)
                    if not os.path.exists(target_dir):
                        os.makedirs(target_dir)
        ...
        for root, dirs, files in os.walk("python"):
            for file in files:
                if file.endswith(".py"):
                    ...
                    target_file = os.path.join(
                        script_directory, "dist/lib", relative_path
                    )
                    target_dir = os.path.dirname(target_file)
                    if not os.path.exists(target_dir):
                        os.makedirs(target_dir)
