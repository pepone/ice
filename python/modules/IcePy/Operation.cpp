...
{
    for (auto p = op->exceptions.begin(); p != op->exceptions.end(); ++p)
    {
...
{
    if ((op->returnType != nullptr || !op->outParams.empty() || !op->exceptions.empty()) && !proxy->ice_isTwoway())
    {
    ...
    try
    {
...

        {
            ...
            ok = _prx->ice_invoke(
                ...
                in,
                out,
                ctx == nullptr || ctx == Py_None ? Ice::noExplicitContext : context);
        }
