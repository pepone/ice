...
declare module "ice" {
    namespace Ice {
        ...
        interface ServantLocator {
             ...
             * This must be done by the servant locator implementation, if this is desired. <code>locate</code> can throw any
             ...
             * client receives {@link UnknownUserException}.
             * If <code>locate</code> throws any exception, the Ice run time does <em>not</em>call <code>finished</code>.
             * <p class="Note">If you call <code>locate</code> from your own code, you must also call <code>finished</code>
             * when you have finished using the servant, provided that <code>locate</code> returned a non-null servant;
             ...
             * <code>finished</code> can throw any user exception. If it does, that exception is marshaled back to the client.
             * If the Slice definition for the corresponding operation includes that user exception, the client receives that
             * user exception; otherwise, the client receives {@link UnknownUserException}.
             * If both the operation and <code>finished</code> throw an exception, the exception thrown by
             * <code>finished</code> is marshaled back to the client.
