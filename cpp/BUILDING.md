...
ZeroC provides [Ice binary distributions][1] for many platforms and compilers, including Windows and Visual Studio,
...
```shell
sudo yum install https://zeroc.com/download/ice/3.7/el10/ice-repo-3.7.el10.noarch.rpm
...
`CXXFLAGS` and `LDFLAGS` variables. For example, to build the Ice C++98 mapping with `-std=c++11`, you can use:
...
Xcode. To build Xcode SDKs, use the `xcodesdk` configurations. The [Ice Builder for Xcode][13] must be installed before
building the SDKs:
...
you also need to add the library directory to your `LD_LIBRARY_PATH` (AIX, Linux) or `DYLD_LIBRARY_PATH` (macOS).
