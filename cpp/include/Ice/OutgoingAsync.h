...
namespace IceInternal
{
    ...
    template<typename Promise> class PromiseInvoke : public virtual OutgoingAsyncCompletionCallback
    {
    public:
        auto getFuture() -> decltype(std::declval<Promise>().get_future())
        {
    ...
    public:
        ...
        void invoke(const std::string& operation, Ice::OperationMode mode, Ice::FormatType format,
                    const Ice::Context& ctx, std::function<void(Ice::OutputStream*)> write,
    ...
    {
    public:
        LambdaOutgoing(const std::shared_ptr<Ice::ObjectPrx>& proxy, std::function<void(R)> response,
                       std::function<void(::std::exception_ptr)> ex, std::function<void(bool)> sent) :
            OutgoingAsyncT<R>(proxy, false),
    ...
    {
    public:
        LambdaOutgoing(const std::shared_ptr<Ice::ObjectPrx>& proxy, std::function<void()> response,
                       std::function<void(::std::exception_ptr)> ex, std::function<void(bool)> sent) :
            OutgoingAsyncT<void>(proxy, false),
            LambdaInvoke(std::move(ex), std::move(sent))
        {
            _response = [this, response](bool ok) {
                if(!ok)
    ...
    {
    public:
        CustomLambdaOutgoing(const std::shared_ptr<Ice::ObjectPrx>& proxy, std::function<void(Ice::InputStream*)> read,
                             std::function<void(::std::exception_ptr)> ex, std::function<void(bool)> sent) :
            OutgoingAsync(proxy, false),
            LambdaInvoke(std::move(ex), std::move(sent))
        {
    ...
    template<typename P, typename R> class PromiseOutgoing : public OutgoingAsyncT<R>, public PromiseInvoke<P>
    {
    public:
        PromiseOutgoing(const std::shared_ptr<Ice::ObjectPrx>& proxy, bool sync) : OutgoingAsyncT<R>(proxy, sync)
        {
    ...
    template<typename P> class PromiseOutgoing<P, void> : public OutgoingAsyncT<void>, public PromiseInvoke<P>
    {
    public:
        PromiseOutgoing(const std::shared_ptr<Ice::ObjectPrx>& proxy, bool sync) : OutgoingAsyncT<void>(proxy, sync)
        {
    ...
    //
    template<class T> class AsyncCallback : public GenericCallbackBase
    {
    public:
    ...
    {
    public:
        CallbackOutgoing(const Ice::ObjectPrx& proxy, const std::string& operation, const CallbackBasePtr& cb,
                         const Ice::LocalObjectPtr& cookie, bool sync) :
            OutgoingAsync(proxy, sync),
...
namespace Ice
{
     ...
     */
    template<class T>
    CallbackPtr newCallback(const IceUtil::Handle<T>& instance, void (T::*cb)(const AsyncResultPtr&),
     ...
     */
    template<class T>
    CallbackPtr newCallback(T* instance, void (T::*cb)(const AsyncResultPtr&),
                            void (T::*sentcb)(const AsyncResultPtr&) = 0)
    {
        return new ::IceInternal::AsyncCallback<T>(instance, cb, sentcb);
    }
