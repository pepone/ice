...
#    else
            ...
            {
                template<class X, class Y> constexpr static bool has_assign(...)
                {
                    return false;
                }
            ...
            {
                template<class X, bool has_any_move_assign> struct has_nothrow_move_assign
                {
                    constexpr static bool value = false;
                };

                template<class X> struct has_nothrow_move_assign<X, true>
                {
...
#    endif
            ...
            namespace detail_
            {
                ...
                {
                    template<class X> constexpr static bool has_overload(...)
                    {
                        return false;
                    }
                ...
                template<typename T, TR2_OPTIONAL_REQUIRES(has_overloaded_addressof<T>)> T* static_addressof(T& ref)
                {
                ...
                // convertible to A
                template<class U> constexpr U convert(U v)
                {
                    return v;
                }
            ...
            template<class T> union storage_t
            {
            ...
            template<class T> union constexpr_storage_t
            {
                unsigned char dummy_;
                T value_;
            ...
            template<class T> struct constexpr_optional_base
            {
                bool init_;
            ...
            template<class T> class optional : private OptionalBase<T>
            {
...
#    endif
            ...
            public:
                ...
                template<class U, class... Args, TR2_OPTIONAL_REQUIRES(is_constructible<T, std::initializer_list<U>>)>
                OPTIONAL_CONSTEXPR_INIT_LIST explicit optional(in_place_t, std::initializer_list<U> il,
                                                               Args&&... args) :
...
#    if OPTIONAL_HAS_THIS_RVALUE_REFS == 1
                ...
                template<class V> constexpr T value_or(V&& v) const&
                {
...
#        if OPTIONAL_HAS_MOVE_ACCESSORS == 1
                ...
                template<class V> OPTIONAL_MUTABLE_CONSTEXPR T value_or(V&& v) &&
                {
...
#        else
                ...
                template<class V> T value_or(V&& v) &&
                {
                    return *this ? constexpr_move(const_cast<optional<T>&>(*this).contained_val()) :
                                   detail_::convert<T>(constexpr_forward<V>(v));
                }
...
#    endif
            ...
            public:
                ...
                template<class V> constexpr typename decay<T>::type value_or(V&& v) const
                {
            ...
            template<class X> constexpr optional<X&> make_optional(reference_wrapper<X> v)
            {
...
namespace std
{
    template<typename T> struct hash<std::experimental::Ice::optional<T>>
    {
