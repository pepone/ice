...
namespace IceMX
{
    ...
    protected:
        ...
        public:
            ...
            ~AttributeResolverT()
            {
                for(typename std::map<std::string, Resolver*>::iterator p = _attributes.begin(); p != _attributes.end();
                    ++p)
                {
                    delete p->second;
                }
            ...
            void setDefault(std::string (Helper::*memberFn)(const std::string&) const)
            {
                _default = memberFn;
            }
        ...
        private:
            ...
            template<typename I, typename V> static I* dynamicCast(V* v)
            {
            ...
            static const std::string toString(const Ice::ObjectPrxPtr& p)
            {
            ...
            static std::string toString(const ::Ice::EndpointPtr& e)
            {
                return e->toString();
            }

            static std::string toString(const ::Ice::ConnectionPtr& e)
            {
                return e->toString();
            }
...
#else
    template<typename T> UpdaterPtr newUpdater(const IceInternal::Handle<T>& updater, void (T::*fn)())
    {
        if(updater)
        {
    ...
    public:
        ...
        ICE_INTERNAL_HANDLE<ObserverImpl> getObserver(const std::string& mapName,
    ...
    template<typename ObserverImplType> class ObserverFactoryT : public Updater, private IceUtil::Mutex
    {
    public:
#ifdef ICE_CPP11_MAPPING
