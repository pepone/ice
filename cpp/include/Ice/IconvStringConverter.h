...
#    endif
    ...
    template<typename charT> IconvStringConverter<charT>::~IconvStringConverter()
    {
    ...
    template<typename charT> std::pair<iconv_t, iconv_t> IconvStringConverter<charT>::createDescriptors() const
    {
    ...
    template<typename charT> /*static*/ void IconvStringConverter<charT>::cleanupKey(void* val)
    {
    ...
    template<typename charT> /*static*/ void IconvStringConverter<charT>::close(std::pair<iconv_t, iconv_t> cdp)
    {
#    ifndef NDEBUG
...
#    endif
    ...
    template<typename charT>
    Ice::Byte* IconvStringConverter<charT>::toUTF8(const charT* sourceStart, const charT* sourceEnd,
...
#    endif
        ...
        if(count == size_t(-1))
        {
            throw Ice::IllegalConversionException(__FILE__, __LINE__, errno != 0 ? strerror(errno) : "Unknown error");
        }
        return reinterpret_cast<Ice::Byte*>(outbuf);
    }

    template<typename charT>
    void IconvStringConverter<charT>::fromUTF8(const Ice::Byte* sourceStart, const Ice::Byte* sourceEnd,
