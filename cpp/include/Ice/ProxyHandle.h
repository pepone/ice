...
namespace IceInternal
{
    template<typename P> P checkedCastImpl(const ::Ice::ObjectPrx&, const ::Ice::Context&);

    template<typename P> P checkedCastImpl(const ::Ice::ObjectPrx&, const std::string&, const ::Ice::Context&);

    template<typename P> P uncheckedCastImpl(const ::Ice::ObjectPrx&);

    template<typename P> P uncheckedCastImpl(const ::Ice::ObjectPrx&, const std::string&);

    //
    // Upcast
    //
    template<typename T, typename Y>
    inline ProxyHandle<T> checkedCastHelper(const ::IceInternal::ProxyHandle<Y>& b, T*, const ::Ice::Context&)
    {
        return b;
    }

    template<typename T, typename Y>
    inline ProxyHandle<T> uncheckedCastHelper(const ::IceInternal::ProxyHandle<Y>& b, T*)
    {
        return b;
    }
    ...
    //
    template<typename T, typename Y>
    inline ProxyHandle<T> checkedCastHelper(const ::IceInternal::ProxyHandle<Y>& b, void*, const ::Ice::Context& ctx)
    {
#    ifdef __SUNPRO_CC
        //
...
#    endif
    ...
    template<typename T, typename Y>
    inline ProxyHandle<T> uncheckedCastHelper(const ::IceInternal::ProxyHandle<Y>& b, void*)
    {
#    ifdef __SUNPRO_CC
        //
        // Sun CC bug introduced in version 5.10
...
#    endif
    ...
    // dynamicCast().
    //
    template<typename T> class ProxyHandle : public ::IceUtil::HandleBase<T>
    {
    public:
        ProxyHandle(T* p = 0)
        {
        ...
        template<typename Y> ProxyHandle(const ProxyHandle<Y>& r)
        {
            this->_ptr = r._ptr;
        ...
        template<typename Y> ProxyHandle(const ::IceUtil::Handle<Y>& r)
        {
            this->_ptr = r._ptr;
        ...
        template<typename Y> ProxyHandle& operator=(const ProxyHandle<Y>& r)
        {
            if(this->_ptr != r._ptr)
            {
                if(r._ptr)
        ...
        template<typename Y> ProxyHandle& operator=(const ::IceUtil::Handle<Y>& r)
        {
            if(this->_ptr != r._ptr)
            {
                if(r._ptr)
...
#    endif
        ...
        template<class Y> static ProxyHandle uncheckedCast(const ProxyHandle<Y>& r)
        {
            Y* tag = 0;
            return ::IceInternal::uncheckedCastHelper<T>(r, tag);
        }

        template<class Y> static ProxyHandle uncheckedCast(const ProxyHandle<Y>& r, const std::string& f)
        {
#    ifdef __SUNPRO_CC
            //
            // Sun CC bug introduced in version 5.10
...
#    endif
    ...
    template<class Y> std::ostream& operator<<(std::ostream& os, ::IceInternal::ProxyHandle<Y> p)
    {
