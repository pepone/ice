...
namespace IceUtil
{
    ...
    public:
         ...
         */
        template<typename Y> Optional(Y p) : _value(p), _isSet(true)
        {
        }
         ...
         */
        template<typename Y> Optional(const Optional<Y>& r) : _value(r._value), _isSet(r._isSet)
        {
        }
         ...
         */
        template<typename Y> Optional& operator=(Y p)
        {
            _value = p;
            _isSet = true;
            return *this;
         ...
         * @param r The source optional.
         */
        template<typename Y> Optional& operator=(const Optional<Y>& r)
        {
            _value = r._value;
            _isSet = r._isSet;
            return *this;
     ...
     */
    template<class T> inline Optional<T> makeOptional(const T& v)
    {
        return Optional<T>(v);
    }

    /// \cond INTERNAL
    template<typename T> inline void Optional<T>::throwOptionalNotSetException(const char* file, int line) const
    {
        throw OptionalNotSetException(file, line);
    }
    /// \endcond

    template<typename T, typename U> inline bool operator==(const Optional<T>& lhs, const Optional<U>& rhs)
    {
        if(lhs && rhs)
        {
            return *lhs == *rhs;
    ...
    template<typename T, typename Y> inline bool operator==(const Optional<T>& lhs, const Y& rhs)
    {
        if(!lhs)
        {
            return false;
    ...
    template<typename T, typename Y> inline bool operator==(const Y& lhs, const Optional<T>& rhs)
    {
        if(!rhs)
        {
            return false;
