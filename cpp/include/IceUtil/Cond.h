...
namespace IceUtil
{
    ...
    public:
        ...
        //
        template<typename Lock> inline void wait(const Lock& lock) const
        {
            if(!lock.acquired())
            {
        ...
        // timeout.
        //
        template<typename Lock> inline bool timedWait(const Lock& lock, const Time& timeout) const
        {
            if(!lock.acquired())
            {
                throw ThreadLockedException(__FILE__, __LINE__);
    ...
    private:
        ...
        template<typename M> void waitImpl(const M& mutex) const
        {
            preWait();
        ...
        template<typename M> bool timedWaitImpl(const M& mutex, const Time& timeout) const
        {
            preWait();
...
#    else
        ...
        enum State{StateIdle, StateSignal, StateBroadcast};
...
#    ifdef ICE_HAS_WIN32_CONDVAR
    ...
    template<typename M> inline void Cond::waitImpl(const M& mutex) const
    {
        typedef typename M::LockState LockState;
    ...
    template<typename M> inline bool Cond::timedWaitImpl(const M& mutex, const Time& timeout) const
    {
