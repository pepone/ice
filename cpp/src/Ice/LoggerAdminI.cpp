...
namespace
{
...
#ifdef ICE_CPP11_MAPPING
        virtual void attachRemoteLogger(shared_ptr<RemoteLoggerPrx>, LogMessageTypeSeq, StringSeq, Int, const Current&);
...
#else
        virtual void attachRemoteLogger(const RemoteLoggerPrx&, const LogMessageTypeSeq&, const StringSeq&, Int,
                                        const Current&);
    ...
    private:
        ...
        {
            RemoteLoggerMap::key_type operator()(const RemoteLoggerMap::value_type& val)
            {
                return val.first;
            }
        };
    ...
    {
    public:
        Job(const vector<RemoteLoggerPrxPtr>& r, const LogMessage& l) : remoteLoggers(r), logMessage(l)
        {
        }
    ...
    //
    void filterLogMessages(LogMessageSeq& logMessages, const set<LogMessageType>& messageTypes,
    ...
    //
    RemoteLoggerPrxPtr changeCommunicator(const RemoteLoggerPrxPtr& prx, const CommunicatorPtr& communicator)
    {
        if(prx == 0)
        {
            return 0;
        ...
        RemoteLoggerPrxPtr result =
            ICE_UNCHECKED_CAST(RemoteLoggerPrx, communicator->stringToProxy(prx->ice_toString()));
    ...
    void
#ifdef ICE_CPP11_MAPPING
    LoggerAdminI::attachRemoteLogger(shared_ptr<RemoteLoggerPrx> prx, LogMessageTypeSeq messageTypes,
                                     StringSeq categories, Int messageMax, const Current& current)
#else
    LoggerAdminI::attachRemoteLogger(const RemoteLoggerPrx& prx, const LogMessageTypeSeq& messageTypes,
                                     const StringSeq& categories, Int messageMax, const Current& current)
#endif
    {
        if(!prx)
        {
        ...
        LoggerAdminLoggerIPtr logger =
            ICE_DYNAMIC_CAST(LoggerAdminLoggerI, current.adapter->getCommunicator()->getLogger());
        if(!logger)
        {
    ...
    LogMessageSeq
#ifdef ICE_CPP11_MAPPING
    LoggerAdminI::getLog(LogMessageTypeSeq messageTypes, StringSeq categories, Int messageMax, string& prefix,
                         const Current& current)
#else
    LoggerAdminI::getLog(const LogMessageTypeSeq& messageTypes, const StringSeq& categories, Int messageMax,
                         string& prefix, const Current& current)
#endif
    {
    ...
    vector<RemoteLoggerPrxPtr> LoggerAdminI::log(const LogMessage& logMessage)
    {
    ...
    void LoggerAdminI::deadRemoteLogger(const RemoteLoggerPrxPtr& remoteLogger, const LoggerPtr& logger,
                                        const LocalException& ex, const string& operation)
    {
        //
    ...
    bool LoggerAdminI::removeRemoteLogger(const RemoteLoggerPrxPtr& remoteLogger)
    {
        IceUtil::Mutex::Lock lock(_mutex);
    ...
    //
    void LoggerAdminI::initCompleted(const AsyncResultPtr& r)
    {
    ...
    {
        LogMessage logMessage = {ICE_ENUM(LogMessageType, PrintMessage), IceUtil::Time::now().toMicroSeconds(), "",
                                 message};
    ...
    {
        LogMessage logMessage = {ICE_ENUM(LogMessageType, TraceMessage), IceUtil::Time::now().toMicroSeconds(),
                                 category, message};
    ...
    {
        LogMessage logMessage = {ICE_ENUM(LogMessageType, WarningMessage), IceUtil::Time::now().toMicroSeconds(), "",
                                 message};
    ...
    {
        LogMessage logMessage = {ICE_ENUM(LogMessageType, ErrorMessage), IceUtil::Time::now().toMicroSeconds(), "",
                                 message};
    ...
    string LoggerAdminLoggerI::getPrefix()
    {
    ...
    ObjectPtr LoggerAdminLoggerI::getFacet() const
    {
        return _loggerAdmin;
    }
    ...
    void LoggerAdminLoggerI::run()
    {
        ...
        for(;;)
        {
            ...
            for(vector<RemoteLoggerPrxPtr>::const_iterator p = job->remoteLoggers.begin();
                ...
                try
                {
                    ...
                    remoteLogger->logAsync(job->logMessage,
                                           ...
                                           [self, remoteLogger](exception_ptr e) {
                                               ...
                                               {
                                                   // expected if there are outstanding calls during communicator
                                                   // destruction
                                               }
                                               catch(const LocalException& ex)
                                               {
                                                   self->_loggerAdmin->deadRemoteLogger(remoteLogger,
                                                                                        self->_localLogger, ex, "log");
                                               }
                                           });
#else
                    //
    ...
    //
    void LoggerAdminLoggerI::logCompleted(const AsyncResultPtr& r)
    {
        RemoteLoggerPrxPtr remoteLogger = ICE_UNCHECKED_CAST(RemoteLoggerPrx, r->getProxy());
