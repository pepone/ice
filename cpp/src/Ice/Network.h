...
typedef BOOL(PASCAL FAR* LPFN_ACCEPTEX)(IN SOCKET sListenSocket, IN SOCKET sAcceptSocket, IN PVOID lpOutputBuffer,
...
//
    ...
    protected:
        ...
        bool checkIfErrorOrCompleted(SocketOperation, Windows::Foundation::IAsyncInfo ^,
                                     Windows::Foundation::AsyncStatus, bool = false);
    ...
    ICE_API std::vector<Address> getAddresses(const std::string&, int, ProtocolSupport, Ice::EndpointSelectionType,
                                              bool, bool);
    ...
    //
    template<typename T> T runSync(Windows::Foundation::IAsyncOperation<T> ^ operation)
    {
        std::promise<T> p;
