...
bool IceBT::isValidDeviceAddress(const string& str)
{
    //
...
bool IceBT::parseDeviceAddress(const string& str, DeviceAddress& addr)
{
...
string IceBT::formatDeviceAddress(const DeviceAddress& addr)
{
    char buf[64];
...
string IceBT::addrToString(const string& addr, Int channel)
{
    ostringstream ostr;
    ostr << addr << '#' << channel;
    return ostr.str();
...
string IceBT::addrToString(const SocketAddress& addr)
{
    return addrToString(formatDeviceAddress(addr.rc_bdaddr), addr.rc_channel);
}

SocketAddress IceBT::createAddr(const string& addr, Ice::Int channel)
{
    SocketAddress ret;
...
string IceBT::fdToString(SOCKET fd)
{
...
void IceBT::fdToAddressAndChannel(SOCKET fd, string& localAddress, int& localChannel, string& remoteAddress,
                                  int& remoteChannel)
{
    if(fd == INVALID_SOCKET)
    {
...
int IceBT::compareAddress(const SocketAddress& addr1, const SocketAddress& addr2)
{
