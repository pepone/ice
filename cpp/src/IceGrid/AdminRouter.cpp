        ...
        void response(bool ok, const pair<const Byte*, const Byte*>& outParams)
        {
            if(_traceLevels->admin > 0)
            {
                ...
                out << "operation `" << _operation << "' routed to `"
                    << Ice::identityToString(_target->ice_getIdentity()) << " -f " << _target->ice_getFacet()
                    << "' is returning ";
                if(ok)
                {
                    out << "successfully";
...
IceGrid::AdminRouter::AdminRouter(const TraceLevelsPtr& traceLevels) : _traceLevels(traceLevels)
{
}

void IceGrid::AdminRouter::invokeOnTarget(const Ice::ObjectPrx& target, const AMD_Object_ice_invokePtr& cb,
                                          const pair<const Byte*, const Byte*>& inParams, const Current& current)
{
    assert(target != 0);
