...
namespace IceGrid
{
    ...
    template<class Function> struct ForEachCommunicator : std::unary_function<CommunicatorDescriptorPtr&, void>
    {
        ForEachCommunicator(Function f) : _function(f)
        {
        }

        void operator()(const ServiceInstanceDescriptor& descriptor)
        {
            assert(descriptor.descriptor);
            operator()(descriptor.descriptor);
        }

        void operator()(const CommunicatorDescriptorPtr& descriptor)
        {
    ...
    template<typename Function> ForEachCommunicator<Function> inline forEachCommunicator(Function function)
    {
        return ForEachCommunicator<Function>(function);
    }

    template<class T, class A> struct ObjFunc : std::unary_function<A, void>
    {
        T& _obj;
    ...
    template<class T, class A> ObjFunc<T, A> inline objFunc(T& obj, void (T::*p)(A))
    {
