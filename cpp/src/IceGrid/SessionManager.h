...
namespace IceGrid
{
    template<class TPrx> class SessionKeepAliveThread : public IceUtil::Thread, public IceUtil::Monitor<IceUtil::Mutex>
    {
        enum State
        {
            Disconnected,
    ...
    public:
        ...
        void waitTryCreateSession(const IceUtil::Time& timeout = IceUtil::Time())
        {
            Lock sync(*this);
        ...
        void setRegistry(const InternalRegistryPrx& registry)
        {
            Lock sync(*this);
            _registry = registry;
        }

        InternalRegistryPrx getRegistry() const
        {
            Lock sync(*this);
            return _registry;
        }
