...
namespace IceGrid
{
    ...
    public:
        SessionReapable(const Ice::LoggerPtr& logger, const TPtr& session) : _logger(logger), _session(session)
        {
        }
        ...
        virtual void destroy(bool shutdown)
        {
            try
            {
                if(shutdown)
    ...
    template<class T> class SessionReapableWithHeartbeat : public SessionReapable<T>
    {
        typedef IceUtil::Handle<T> TPtr;
