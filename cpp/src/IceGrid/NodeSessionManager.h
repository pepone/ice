...
namespace IceGrid
{
    ...
    public:
        ...
        {
            return _thread->getSession();
        }
        std::vector<IceGrid::QueryPrx> getQueryObjects()
        {
            return findAllQueryObjects(true);
        }
    ...
    private:
        ...
        public:
            ...
            virtual NodeSessionPrx createSession(InternalRegistryPrx& master, IceUtil::Time& timeout)
            {
            ...
            virtual void destroySession(const NodeSessionPrx& session)
            {
                NodeSessionKeepAliveThread::destroySession(session);
                _manager.reapReplicas();
            }

            virtual bool keepAlive(const NodeSessionPrx& session)
            {
