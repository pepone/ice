...
namespace IceGrid
{
    ...
    {
    public:
        AdapterRequest(const Ice::AMD_Locator_findAdapterByIdPtr& amdCB, const LocatorIPtr& locator,
                       const Ice::EncodingVersion& encoding, const LocatorAdapterInfo& adapter) :
            _amdCB(amdCB),
            _locator(locator),
            _encoding(encoding),
            _adapter(adapter),
        ...
        virtual void activating(const string&)
        {
            // Nothing to do.
        }

        virtual void response(const std::string& id, const Ice::ObjectPrx& proxy)
        {
            assert(proxy);
            ...
            {
                exception(id, Ice::UnsupportedEncodingException(__FILE__, __LINE__,
                                                                "server doesn't support requested encoding", _encoding,
        ...
        virtual void exception(const std::string&, const Ice::Exception& ex)
        {
    ...
    {
    public:
        ReplicaGroupRequest(const Ice::AMD_Locator_findAdapterByIdPtr& amdCB, const LocatorIPtr& locator,
                            const string& id, const Ice::EncodingVersion& encoding,
                            const LocatorAdapterInfoSeq& adapters, int count, Ice::ObjectPrx firstProxy) :
            _amdCB(amdCB),
            _locator(locator),
            _id(id),
            _encoding(encoding),
        ...
        virtual void exception(const string& /*id*/, const Ice::Exception& ex)
        {
            LocatorAdapterInfo adapter;
            {
                Lock sync(*this);
        ...
        virtual void response(const string& id, const Ice::ObjectPrx& proxy)
        {
            //
            // Ensure the server supports the request encoding.
            //
    ...
    {
    public:
        RoundRobinRequest(const Ice::AMD_Locator_findAdapterByIdPtr& amdCB, const LocatorIPtr& locator,
                          const DatabasePtr database, const string& id, const Ice::Current& current,
                          const LocatorAdapterInfoSeq& adapters, int count) :
            _amdCB(amdCB),
            _locator(locator),
            _database(database),
            _id(id),
        ...
        virtual void activating(const string& id)
        {
            LocatorAdapterInfo adapter;
            adapter.id = id;
            do
        ...
        virtual void exception(const std::string& id, const Ice::Exception& ex)
        {
            LocatorAdapterInfo adapter;
            {
                Lock sync(*this);
    ...
    {
    public:
        FindAdapterByIdCallback(const LocatorIPtr& locator, const Ice::AMD_Locator_findAdapterByIdPtr& cb,
                                const string& id, const Ice::Current& current) :
            _locator(locator),
            _cb(cb),
            _id(id),
            _current(current)
...
LocatorI::LocatorI(const Ice::CommunicatorPtr& communicator, const DatabasePtr& database,
                   const WellKnownObjectsManagerPtr& wellKnownObjects, const RegistryPrx& registry,
...
// registry.
//
void LocatorI::findObjectById_async(const Ice::AMD_Locator_findObjectByIdPtr& cb, const Ice::Identity& id,
...
//
void LocatorI::findAdapterById_async(const Ice::AMD_Locator_findAdapterByIdPtr& cb, const string& id,
...
Ice::LocatorRegistryPrx LocatorI::getRegistry(const Ice::Current&) const
{
    return _wellKnownObjects->getLocatorRegistry();
}

RegistryPrx LocatorI::getLocalRegistry(const Ice::Current&) const
{
    return _localRegistry;
}

QueryPrx LocatorI::getLocalQuery(const Ice::Current&) const
{
    return _localQuery;
}

const Ice::CommunicatorPtr& LocatorI::getCommunicator() const
{
    return _communicator;
}

const TraceLevelsPtr& LocatorI::getTraceLevels() const
{
    return _database->getTraceLevels();
}

bool LocatorI::getDirectProxy(const LocatorAdapterInfo& adapter, const RequestPtr& request)
{
    {
        Lock sync(*this);
    ...
    adapter.proxy->begin_getDirectProxy(newCallback_Adapter_getDirectProxy(
        new AdapterGetDirectProxyCallback(this, adapter), &AdapterGetDirectProxyCallback::response,
        &AdapterGetDirectProxyCallback::exception));
    return false;
}

void LocatorI::getDirectProxyResponse(const LocatorAdapterInfo& adapter, const Ice::ObjectPrx& proxy)
{
    PendingRequests requests;
    {
        Lock sync(*this);
...
void LocatorI::getDirectProxyException(const LocatorAdapterInfo& adapter, const Ice::Exception& ex)
{
    bool activate = false;
    try
    {
    ...
    if(activate)
    {
        ...
        AdapterPrx::uncheckedCast(adapter.proxy->ice_invocationTimeout(timeout * 1000))
            ->begin_activate(newCallback_Adapter_activate(new AdapterActivateCallback(this, adapter),
