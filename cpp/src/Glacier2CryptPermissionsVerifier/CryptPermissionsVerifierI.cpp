...
namespace
{
    ...
    bool CryptPermissionsVerifierI::checkPermissions(const string& userId, const string& password, string&,
                                                     const Current&) const
    {
...
#    if defined(__APPLE__)
        ...
        UniqueRef<CFDataRef> data(CFDataCreateWithBytesNoCopy(
            kCFAllocatorDefault, reinterpret_cast<const uint8_t*>(salt.c_str()), salt.size(), kCFAllocatorNull));
