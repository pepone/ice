...
namespace
{
    ...
    PatcherI::PatcherI(const FileServerPrx& server, const PatcherFeedbackPtr& feedback, const string& dataDir,
                       bool thorough, Ice::Int chunkSize, Ice::Int remove) :
        _feedback(feedback),
        _dataDir(dataDir),
        _thorough(thorough),
    ...
    void PatcherI::init(const FileServerPrx& server)
    {
        if(_dataDir.empty())
        {
                        ...
                        while(pos < p->size)
                        {
                            if(!curCB)
                            {
                                ...
                                curCB = _useSmallFileAPI ?
                                            _serverNoCompress->begin_getFileCompressed(
                                                p->path, static_cast<Ice::Int>(pos), _chunkSize) :
                            ...
                            if(pos + _chunkSize < p->size)
                            {
                                nxtCB = _useSmallFileAPI ?
                                            _serverNoCompress->begin_getFileCompressed(
                                                p->path, static_cast<Ice::Int>(pos + _chunkSize), _chunkSize) :
                                            _serverNoCompress->begin_getLargeFileCompressed(p->path, pos + _chunkSize,
                                                                                            _chunkSize);
                            }
                            else
...
PatcherPtr PatcherFactory::create(const Ice::CommunicatorPtr& communicator, const PatcherFeedbackPtr& feedback)
{
...
//
PatcherPtr PatcherFactory::create(const FileServerPrx& server, const PatcherFeedbackPtr& feedback,
                                  const string& dataDir, bool thorough, Ice::Int chunkSize, Ice::Int remove)
{
