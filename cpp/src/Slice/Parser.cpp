...
Slice::Unit::setCurrentFile(std::string currentFile, int lineNumber)
{
    ...
    if (IceInternal::isAbsolutePath(currentFile))
    {
        ...
        if (!IceInternal::fileExists(absolutePath))
        {
            ...
            // If the file is an absolute path, and it does not exists, it must be relative to the
            // current definition context.
            string previousDir = dirName(dc->filename());
    ...
    else
    {
        ...
        string dirName = Slice::dirName(_parentFile);
