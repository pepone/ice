...
void LocatorRegistryI::setAdapterDirectProxyAsync(string adapterId, shared_ptr<ObjectPrx> proxy,
                                                  function<void()> response, function<void(exception_ptr)>,
                                                  const Ice::Current&)
#else
void LocatorRegistryI::setAdapterDirectProxy_async(const AMD_LocatorRegistry_setAdapterDirectProxyPtr& cb,
                                                   const std::string& adapterId, const ObjectPrxPtr& proxy,
                                                   const Current&)
#endif
{
    Lock sync(*this);
...
#ifdef ICE_CPP11_MAPPING
void LocatorRegistryI::setReplicatedAdapterDirectProxyAsync(string adapterId, string replicaGroupId,
                                                            shared_ptr<ObjectPrx> proxy, function<void()> response,
                                                            function<void(exception_ptr)>, const Ice::Current&)
#else
void LocatorRegistryI::setReplicatedAdapterDirectProxy_async(
    const AMD_LocatorRegistry_setReplicatedAdapterDirectProxyPtr& cb, const std::string& adapterId,
    const std::string& replicaGroupId, const ObjectPrxPtr& proxy, const Current&)
#endif
{
    Lock sync(*this);
    if(proxy)
...
#ifdef ICE_CPP11_MAPPING
void LocatorRegistryI::setServerProcessProxyAsync(string, shared_ptr<ProcessPrx>, function<void()> response,
                                                  function<void(exception_ptr)>, const Ice::Current&)
{
    response();
}
#else
void LocatorRegistryI::setServerProcessProxy_async(const AMD_LocatorRegistry_setServerProcessProxyPtr& cb,
                                                   const std::string&, const ProcessPrxPtr&, const Current&)
{
    cb->ice_response();
}
#endif

Ice::ObjectPrxPtr LocatorRegistryI::findObject(const Ice::Identity& id) const
{
    Lock sync(*this);
    if(id.name.empty())
    {
...
Ice::ObjectPrxPtr LocatorRegistryI::findAdapter(const string& adapterId, bool& isReplicaGroup) const
{
    Lock sync(*this);
...
LocatorI::LocatorI(const LookupIPtr& lookup, const LocatorRegistryPrxPtr& registry) :
    _lookup(lookup),
    _registry(registry)
{
}
...
#ifdef ICE_CPP11_MAPPING
void LocatorI::findObjectByIdAsync(Ice::Identity id, function<void(const shared_ptr<ObjectPrx>&)> response,
                                   function<void(exception_ptr)> ex, const Ice::Current&) const
{
    _lookup->findObject(make_pair(response, ex), id);
}

void LocatorI::findAdapterByIdAsync(string adapterId, function<void(const shared_ptr<ObjectPrx>&)> response,
                                    function<void(exception_ptr)> ex, const Ice::Current&) const
{
    _lookup->findAdapter(make_pair(response, ex), adapterId);
}
#else
void LocatorI::findObjectById_async(const AMD_Locator_findObjectByIdPtr& cb, const Identity& id, const Current&) const
{
    _lookup->findObject(cb, id);
}

void LocatorI::findAdapterById_async(const AMD_Locator_findAdapterByIdPtr& cb, const std::string& adapterId,
                                     const Current&) const
{
...
LocatorRegistryPrxPtr LocatorI::getRegistry(const Current&) const
{
    return _registry;
}
