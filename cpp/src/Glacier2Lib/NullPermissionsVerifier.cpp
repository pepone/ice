...
namespace
{
    ...
    public:
#ifdef ICE_CPP11_MAPPING
        virtual bool authorize(Glacier2::SSLInfo, string&, const Ice::Current&) const
#else
        virtual bool authorize(const Glacier2::SSLInfo&, string&, const Ice::Current&) const
#endif
        {
            return true;
        }
    ...
    string Init::checkPermissionVerifier(const string& val)
    {
        ...
        catch(const ProxyParseException&)
        {
            ...
            {
                createObjects();
                return _adapter->createProxy(_nullSSLPVId)
                    ->ice_toString(); // Return valid proxy to rewrite the property
            }
...
{
    void setupNullPermissionsVerifier(const CommunicatorPtr& communicator, const string& category,
