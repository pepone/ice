...
namespace
{
    ...
    class SChannelCertificateI : public SChannel::Certificate, public CertificateI, public IceUtil::Mutex
    {
    public:
    ...
    string certNameToString(CERT_NAME_BLOB* name)
    {
        assert(name);
        DWORD length = 0;
...
SCHannelX509ExtensionI::SCHannelX509ExtensionI(CERT_EXTENSION extension, const string& oid,
                                               const CertInfoHolderPtr& ci) :
...
bool SCHannelX509ExtensionI::isCritical() const
{
    return _extension.fCritical != 0;
}

string SCHannelX509ExtensionI::getOID() const
{
    return _oid;
}

vector<Ice::Byte> SCHannelX509ExtensionI::getData() const
{
...
bool SChannelCertificateI::operator==(const IceSSL::Certificate& r) const
{
...
vector<Ice::Byte> SChannelCertificateI::getAuthorityKeyIdentifier() const
{
...
vector<Ice::Byte> SChannelCertificateI::getSubjectKeyIdentifier() const
{
    vector<Ice::Byte> keyid;
    PCERT_EXTENSION extension =
...
bool SChannelCertificateI::verify(const CertificatePtr& cert) const
{
    bool result = false;
...
string SChannelCertificateI::encode() const
{
    string s;
    DWORD length = 0;
    BYTE* buffer = 0;
...
string SChannelCertificateI::getSerialNumber() const
{
    ostringstream os;
...
DistinguishedName SChannelCertificateI::getIssuerDN() const
{
    return DistinguishedName(certNameToString(&_certInfo->Issuer));
}

vector<pair<int, string>> SChannelCertificateI::getIssuerAlternativeNames() const
{
    return certificateAltNames(_certInfo, szOID_ISSUER_ALT_NAME2);
}

DistinguishedName SChannelCertificateI::getSubjectDN() const
{
    return DistinguishedName(certNameToString(&_certInfo->Subject));
}

vector<pair<int, string>> SChannelCertificateI::getSubjectAlternativeNames() const
{
...
CERT_SIGNED_CONTENT_INFO* SChannelCertificateI::getCert() const
{
    return _cert;
}

void SChannelCertificateI::loadX509Extensions() const
{
    ...
    if(_extensions.empty())
    {
        for(size_t i = 0; i < _certInfo->cExtension; ++i)
        {
            ...
            _extensions.push_back(ICE_DYNAMIC_CAST(
                X509Extension, ICE_MAKE_SHARED(SCHannelX509ExtensionI, ext, ext.pszObjId, _certInfoHolder)));
        }
    }
}

SChannel::CertificatePtr SChannel::Certificate::create(CERT_SIGNED_CONTENT_INFO* cert)
{
    return ICE_MAKE_SHARED(SChannelCertificateI, cert);
}

SChannel::CertificatePtr SChannel::Certificate::load(const std::string& file)
{
...
SChannel::CertificatePtr SChannel::Certificate::decode(const std::string& encoding)
{
    CERT_SIGNED_CONTENT_INFO* cert;
