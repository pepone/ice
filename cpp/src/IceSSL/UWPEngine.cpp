...
IceUtil::Shared* UWP::upCast(UWP::SSLEngine* p)
{
    return p;
}
...
namespace
{
    ...
    Certificates::Certificate ^
        ...
        //
        bool importPfxData(String ^ friendlyName, String ^ data, String ^ password)
    {
        try
        {
            IceInternal::runSync(CertificateEnrollmentManager::ImportPfxDataAsync(
                data, password, ExportOption::NotExportable, KeyProtectionLevel::NoConsent, InstallOptions::None,
                friendlyName));
        ...
        importPersonalCertificate(const string& file, function<string()> password, bool passwordPrompt,
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        passwordErr =
                            importPfxData(friendlyName, data, ref new String(stringToWstring(password()).c_str()));
            ...
            if(value != "*")
            {
                ...
                while((pos = value.find(':', start)) != string::npos)
                {
                    ...
                    else if(field == "THUMBPRINT")
                    {
                        ...
                        query->Thumbprint =
                            ref new Array<unsigned char>(&buffer[0], static_cast<unsigned int>(buffer.size()));
                    }
                }
            }
...
void UWP::SSLEngine::initialize()
{
    ...
    if(!certFile.empty())
    {
        _certificate = dynamic_pointer_cast<UWP::Certificate>(UWP::Certificate::create(importPersonalCertificate(
            certFile, [this]() { return password(false); }, getPasswordPrompt() != nullptr, passwordRetryMax)));
    }
...
bool UWP::SSLEngine::initialized() const
{
    return _initialized;
}

shared_ptr<UWP::Certificate> UWP::SSLEngine::certificate()
{
    return _certificate;
}

void UWP::SSLEngine::destroy()
{
}

IceInternal::TransceiverPtr UWP::SSLEngine::createTransceiver(const InstancePtr& instance,
