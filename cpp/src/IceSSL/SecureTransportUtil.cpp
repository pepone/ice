...
string IceSSL::SecureTransport::sslErrorToString(CFErrorRef err)
{
    ostringstream os;
    if(err)
    {
...
string IceSSL::SecureTransport::sslErrorToString(OSStatus status)
{
    ostringstream os;
    os << "(error: " << status;
...
#if defined(ICE_USE_SECURE_TRANSPORT_MACOS)
CFDictionaryRef IceSSL::SecureTransport::getCertificateProperty(SecCertificateRef cert, CFTypeRef key)
{
...
namespace
{
    ...
    //
    CFArrayRef loadKeychainItems(const string& file, SecExternalItemType type, SecKeychainRef keychain,
                                 const string& passphrase, const PasswordPromptPtr& prompt, int retryMax)
    {
    ...
    SecKeychainRef openKeychain(const std::string& path, const std::string& keychainPassword)
    {
        ...
        else if(err == errSecNoSuchKeychain)
        {
            const char* pass = keychainPassword.empty() ? 0 : keychainPassword.c_str();
            keychain.reset(0);
            if((err = SecKeychainCreate(keychainPath.c_str(), keychainPassword.size(), pass, pass == 0, 0,
                                        &keychain.get())))
            {
                throw PluginInitializationException(__FILE__, __LINE__,
    ...
    //
    SecIdentityRef loadPrivateKey(const string& file, SecCertificateRef cert, SecKeychainRef keychain,
...
//
CFArrayRef IceSSL::SecureTransport::loadCertificateChain(const string& file, const string& keyFile,
                                                         const std::string& keychainPath,
                                                         const string& keychainPassword, const string& password,
...
SecCertificateRef IceSSL::SecureTransport::loadCertificate(const string& file)
{
...
CFArrayRef IceSSL::SecureTransport::loadCACertificates(const string& file)
{
#if defined(ICE_USE_SECURE_TRANSPORT_IOS)
...
CFArrayRef IceSSL::SecureTransport::findCertificateChain(const std::string& keychainPath,
                                                         const std::string& keychainPassword, const string& value)
{
    //
