...
testFindCert(const string& factoryRef, const string& defaultDir, const Ice::PropertiesPtr& defaultProps, bool p12)
{
    ...
    const char* clientFindCertProperties[] = {
        //            "SUBJECT:Client",
        "LABEL:Client",
        "SUBJECTKEYID:'D5 C0 78 2A D5 37 45 3B 7C E0 AD 03 68 13 87 2A D8 A8 28 56'",
        "SERIAL:'10 01'",
        "SERIAL:'10 01' LABEL:Client",
        nullptr};
...
#    endif
        "LABEL:Server",
        "SUBJECTKEYID:'1C 3E 5A 58 8D E7 54 5F 61 51 E7 51 DD 3C 09 48 E7 2A ED 1B'",
        "SERIAL:'10 0C'",
        "SERIAL:'10 0C' LABEL:Server",
        nullptr};
    ...
    bool p12)
{
    ...
    // The client can't verify the server certificate but it should still provide it. "ca1/server"
    // doesn't include the root so the cert size should be 1.
    ...
    // Setting the CA for the server shouldn't change anything, it shouldn't modify the cert chain sent to the
    // client.
    d = createServerProps(defaultProps, p12, "ca1/server", "ca1/ca1");
    d["IceSSL.VerifyPeer"] = "0";
    server = fact->createServer(d);
