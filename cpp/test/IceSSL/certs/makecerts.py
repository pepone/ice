...
for ca, name in [
    ...
    if force or not os.path.exists(pem):
        ca.getCA().save(pem)
    der = "{}.der".format(name)
    if force or not os.path.exists(der):
        ca.getCA().save(der)
...
if force or not os.path.exists("cacerts.pem"):
    ...
    with open("cacert2.pem", "r") as f:
        pem += f.read()
    with open("cacert3.pem", "r") as f:
        pem += f.read()
    with open("cacert4.pem", "r") as f:
        pem += f.read()
...
if force or not os.path.exists("cacert2_pub.pem"):
    ca2.getCA().save("cacert2_pub.pem")
if force or not os.path.exists("cacert2_priv.pem"):
    ca2.getCA().saveKey("cacert2_priv.pem")
...
if force or not os.path.exists("cai3.der"):
    cai3.getCA().save("cai3.der")
...
if force or not os.path.exists("cai4.der"):
    cai4.getCA().save("cai4.der")
