...
namespace
{
    ...
    public:
        ...
        void closed(const Ice::ConnectionPtr& connection)
        {
    ...
    public:
        ...
        void heartbeat(const Ice::ConnectionPtr& connection)
        {
...
void MyClassI::callCallback_async(const AMD_MyClass_callCallbackPtr& amdCB, const Ice::Current& c)
{
    checkConnection(c.con);
    Callback_Callback_pingPtr cb =
        newCallback_Callback_ping(new CallbackI(amdCB), &CallbackI::response, &CallbackI::exception);
    Ice::uncheckedCast<CallbackPrx>(c.con->createProxy(callbackId))->begin_ping(cb);
}

void MyClassI::getCallbackCount_async(const AMD_MyClass_getCallbackCountPtr& amdCB, const Ice::Current& c)
{
    checkConnection(c.con);
    Callback_Callback_getCountPtr cb = newCallback_Callback_getCount(
        new GetCallbackCount(amdCB), &GetCallbackCount::response, &GetCallbackCount::exception);
    Ice::uncheckedCast<CallbackPrx>(c.con->createProxy(callbackId))->begin_getCount(cb);
}

void MyClassI::incCounter(int expected, const Ice::Current& c)
{
    checkConnection(c.con);
...
void MyClassI::waitCounter(int value, const Ice::Current& c)
{
    IceUtil::Monitor<IceUtil::Mutex>::Lock sync(_monitor);
...
int MyClassI::getConnectionCount(const Ice::Current& c)
{
    checkConnection(c.con);
    return static_cast<int>(_connections.size());
}

string MyClassI::getConnectionInfo(const Ice::Current& c)
{
    checkConnection(c.con);
    return c.con->toString();
}

void MyClassI::closeConnection(bool forceful, const Ice::Current& c)
{
    checkConnection(c.con);
    if(forceful)
    {
...
void MyClassI::datagram(const Ice::Current& c)
{
    checkConnection(c.con);
...
int MyClassI::getDatagramCount(const Ice::Current& c)
{
    checkConnection(c.con);
    return _datagramCount;
}

void MyClassI::callDatagramCallback(const Ice::Current& c)
{
    checkConnection(c.con);
    test(c.con->getEndpoint()->getInfo()->datagram());
...
void MyClassI::getCallbackDatagramCount_async(const AMD_MyClass_getCallbackDatagramCountPtr& amdCB,
                                              const Ice::Current& c)
{
    checkConnection(c.con);
    Callback_Callback_getDatagramCountPtr cb = newCallback_Callback_getDatagramCount(
        new GetCallbackDatagramCount(amdCB), &GetCallbackDatagramCount::response, &GetCallbackDatagramCount::exception);
    Ice::uncheckedCast<CallbackPrx>(c.con->createProxy(callbackId))->begin_getDatagramCount(cb);
}

int MyClassI::getHeartbeatCount(const Ice::Current& c)
{
    checkConnection(c.con);
...
void MyClassI::enableHeartbeats(const Ice::Current& c)
{
    checkConnection(c.con);
    c.con->setACM(1, IceUtil::None, Ice::HeartbeatAlways);
}

void MyClassI::shutdown(const Ice::Current& c)
{
    checkConnection(c.con);
    c.adapter->getCommunicator()->shutdown();
}

void MyClassI::removeConnection(const Ice::ConnectionPtr& c)
{
    IceUtil::Monitor<IceUtil::Mutex>::Lock sync(_monitor);
    _connections.erase(c);
}

void MyClassI::incHeartbeatCount(const Ice::ConnectionPtr& c)
{
    IceUtil::Monitor<IceUtil::Mutex>::Lock sync(_monitor);
...
void MyClassI::checkConnection(const Ice::ConnectionPtr& c)
{
    IceUtil::Monitor<IceUtil::Mutex>::Lock sync(_monitor);
