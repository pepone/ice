...
namespace
{
    ...
    public:
        ...
        virtual void run()
        {
            TestIntfPrxPtr proxy =
                ICE_UNCHECKED_CAST(TestIntfPrx, _communicator->stringToProxy(_adapter->getTestIntf()->ice_toString()));
            try
            {
#ifdef ICE_CPP11_MAPPING
                auto self = shared_from_this();
                proxy->ice_getConnection()->setCloseCallback(
                    [self](Ice::ConnectionPtr connection) { self->closed(move(connection)); });
                proxy->ice_getConnection()->setHeartbeatCallback(
                    [self](Ice::ConnectionPtr connection) { self->heartbeat(move(connection)); });
#else
...
void allTests(const Ice::CommunicatorPtr& communicator)
{
    ...
    for(auto p = tests.begin(); p != tests.end(); ++p)
    {
        ...
        thread t([testCase]() { testCase->run(); });
