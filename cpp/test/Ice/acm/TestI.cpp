...
RemoteObjectAdapterPrxPtr RemoteCommunicatorI::createObjectAdapter(int timeout, int close, int heartbeat,
...
void RemoteObjectAdapterI::activate(const Ice::Current&)
{
    _adapter->activate();
}

void RemoteObjectAdapterI::hold(const Ice::Current&)
{
    _adapter->hold();
}

void RemoteObjectAdapterI::deactivate(const Ice::Current&)
{
    try
    {
        _adapter->destroy();
...
void TestI::sleep(int delay, const Ice::Current& current)
{
    Lock sync(*this);
    timedWait(IceUtil::Time::seconds(delay));
}

void TestI::sleepAndHold(int delay, const Ice::Current& current)
{
    Lock sync(*this);
...
void TestI::interruptSleep(const Ice::Current& current)
{
    Lock sync(*this);
    notifyAll();
}

void TestI::startHeartbeatCount(const Ice::Current& current)
{
    _callback = ICE_MAKE_SHARED(HeartbeatCallbackI);
#ifdef ICE_CPP11_MAPPING
    HeartbeatCallbackIPtr callback = _callback;
    current.con->setHeartbeatCallback(
        [callback](Ice::ConnectionPtr connection) { callback->heartbeat(move(connection)); });
#else
    current.con->setHeartbeatCallback(_callback);
#endif
}

void TestI::waitForHeartbeatCount(int count, const Ice::Current&)
{
    assert(_callback);
