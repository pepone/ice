...
namespace
{
    ...
    public:
        ...
        void ignoreEx(const Ice::Exception& ex)
        {
...
void allTests(const Ice::CommunicatorPtr& communicator)
{
    ...
    cout << "testing dispatcher... " << flush;
    {
...

        {
            ...
            to->sleepAsync(500, [cb]() { cb->responseEx(); },
                           [cb](exception_ptr err) {
                               try
                               {
                                   rethrow_exception(err);
        ...
        while(true)
        {
            ...
            p->opWithPayloadAsync(seq, [=]() { c->set_value(); }, [=](exception_ptr) { c->set_value(); },
...
#else
        Test::Callback_TestIntf_opPtr callback =
            Test::newCallback_TestIntf_op(cb, &Callback::response, &Callback::exception);
        p->begin_op(callback);
        cb->check();
