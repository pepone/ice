...
allTests(Test::TestHelper* helper)
{
    ...
    auto obj = Ice::uncheckedCast<TestIntfPrx>(
        communicator->stringToProxy("test:" + helper->getTestEndpoint()).value());
    ...
    cout << "testing object adapter published endpoints... " << flush;
    {
        ...
        Ice::ObjectPrx prx = communicator->stringToProxy(
            "dummy:tcp -h localhost -p 12346 -t 20000:tcp -h localhost -p 12347 -t 10000").value();
    ...
    cout << "testing object adapter with router... " << flush;
    {
        ...
        auto router = Ice::uncheckedCast<Ice::RouterPrx>(obj->ice_identity(routerId)->ice_connectionId("rc"));
        ...
        {
            routerId.name = "test";
            router = Ice::uncheckedCast<Ice::RouterPrx>(obj->ice_identity(routerId));
