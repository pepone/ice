...
void ::Reader::run(int argc, char* argv[])
{
...

    {
        ...
        // Read 100 samples from the "element" key and close the connnection.
        for (int i = 0; i < 100; ++i)
        {
            auto sample = reader.getNextUnread();
            if (sample.getValue() != i)
        ...
        // Send a sample to the writer on "reader_barrier" to let it know connection was closed.
        ...
        // Wait for the writer to acknowledge the sample send on "reader_barrier" and close the
        // connection again.
