...
template<class T> class CookieT : public Cookie
{
public:
...
{
public:
    void responseCallback(const CookiePtr& cookie)
    {
        getCookie<AMD_Callback_initiateCallbackPtr>(cookie)->ice_response();
    }

    void exceptionCallback(const Ice::Exception& ex, const CookiePtr& cookie)
    {
        getCookie<AMD_Callback_initiateCallbackPtr>(cookie)->ice_exception(ex);
    }

    void responseCallbackEx(const CookiePtr& cookie)
    {
        getCookie<AMD_Callback_initiateCallbackExPtr>(cookie)->ice_response();
    }

    void exceptionCallbackEx(const Ice::Exception& ex, const CookiePtr& cookie)
    {
        getCookie<AMD_Callback_initiateCallbackExPtr>(cookie)->ice_exception(ex);
    }

    void responseConcurrentCallback(Int number, const CookiePtr& cookie)
    {
        getCookie<AMD_Callback_initiateConcurrentCallbackPtr>(cookie)->ice_response(number);
    }

    void exceptionConcurrentCallback(const Ice::Exception& ex, const CookiePtr& cookie)
    {
        getCookie<AMD_Callback_initiateConcurrentCallbackPtr>(cookie)->ice_exception(ex);
    }

    void responseWaitCallback(const CookiePtr& cookie)
    {
        getCookie<AMD_Callback_initiateWaitCallbackPtr>(cookie)->ice_response();
    }

    void exceptionWaitCallback(const Ice::Exception& ex, const CookiePtr& cookie)
    {
        getCookie<AMD_Callback_initiateWaitCallbackPtr>(cookie)->ice_exception(ex);
    }

    void responseCallbackWithPayload(const CookiePtr& cookie)
    {
        getCookie<AMD_Callback_initiateCallbackWithPayloadPtr>(cookie)->ice_response();
    }

    void exceptionCallbackWithPayload(const Ice::Exception& ex, const CookiePtr& cookie)
    {
...
void CallbackReceiverI::callback(const Current&)
{
    Lock sync(*this);
    ++_callback;
    notifyAll();
...
void CallbackReceiverI::callbackEx(const Current& current)
{
    callback(current);
...
void CallbackReceiverI::concurrentCallback_async(const AMD_CallbackReceiver_concurrentCallbackPtr& cb, Int number,
                                                 const Current&)
{
    Lock sync(*this);
...
void CallbackReceiverI::waitCallback(const Current&)
{
    {
        Lock sync(*this);
...
void CallbackReceiverI::callbackWithPayload(const Ice::ByteSeq&, const Current&)
{
    Lock sync(*this);
...
void CallbackReceiverI::callbackOK(int expected)
{
    Lock sync(*this);
...
void CallbackReceiverI::waitCallbackOK()
{
    Lock sync(*this);
    while(!_waitCallback)
    {
...
void CallbackReceiverI::callbackWithPayloadOK()
{
    Lock sync(*this);
...
void CallbackReceiverI::notifyWaitCallback()
{
    Lock sync(*this);
...
void CallbackReceiverI::answerConcurrentCallbacks(unsigned int num)
{
    Lock sync(*this);
...
void CallbackI::initiateCallback_async(const AMD_Callback_initiateCallbackPtr& cb, const CallbackReceiverPrx& proxy,
                                       const Current& current)
{
    if(proxy->ice_isTwoway())
    {
        AsyncCBPtr acb = new AsyncCB();
        proxy->begin_callback(
            current.ctx,
...
void CallbackI::initiateCallbackEx_async(const AMD_Callback_initiateCallbackExPtr& cb, const CallbackReceiverPrx& proxy,
                                         const Current& current)
{
    if(proxy->ice_isTwoway())
    {
        AsyncCBPtr acb = new AsyncCB();
        proxy->begin_callbackEx(
            current.ctx,
...
void CallbackI::initiateConcurrentCallback_async(const AMD_Callback_initiateConcurrentCallbackPtr& cb, Int number,
                                                 const CallbackReceiverPrx& proxy, const Current& current)
{
    AsyncCBPtr acb = new AsyncCB();
    proxy->begin_concurrentCallback(
        number, current.ctx,
...
void CallbackI::initiateWaitCallback_async(const AMD_Callback_initiateWaitCallbackPtr& cb,
                                           const CallbackReceiverPrx& proxy, const Current& current)
{
    AsyncCBPtr acb = new AsyncCB();
    proxy->begin_waitCallback(
        current.ctx,
        newCallback_CallbackReceiver_waitCallback(acb, &AsyncCB::responseWaitCallback, &AsyncCB::exceptionWaitCallback),
        newCookie(cb));
}

void CallbackI::initiateCallbackWithPayload_async(const AMD_Callback_initiateCallbackWithPayloadPtr& cb,
                                                  const CallbackReceiverPrx& proxy, const Current& current)
{
    Ice::ByteSeq seq(1000 * 1024, 0);
    AsyncCBPtr acb = new AsyncCB();
    proxy->begin_callbackWithPayload(
        seq, current.ctx,
...
void CallbackI::shutdown(const Ice::Current& current)
{
