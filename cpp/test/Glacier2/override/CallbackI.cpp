...
CallbackReceiverI::CallbackReceiverI() : _holding(false), _lastToken(-1), _callback(0), _callbackWithPayload(0)
{
}

void CallbackReceiverI::callback(int token, const Current&)
{
    Lock sync(*this);
    checkForHold();
...
int CallbackReceiverI::callbackOK(int count, int token)
{
    Lock sync(*this);
...
int CallbackReceiverI::callbackWithPayloadOK(int count)
{
    Lock sync(*this);
...
void CallbackReceiverI::hold()
{
    Lock sync(*this);
    _holding = true;
}

void CallbackReceiverI::activate()
{
    Lock sync(*this);
    _holding = false;
    notifyAll();
...
void CallbackReceiverI::checkForHold()
{
    while(_holding)
    {
        wait();
...
void CallbackI::initiateCallback_async(const AMD_Callback_initiateCallbackPtr& cb, const CallbackReceiverPrx& proxy,
                                       int token, const Current& current)
{
