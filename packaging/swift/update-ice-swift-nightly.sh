...
# Update the Package.swift file with the URL and Checksum for the slice2swift zip
for zip_file in "${STAGING_DIR}"/slice2swift-*.zip; do
    echo "Processing slice2swift zip file: ${zip_file}"
    zip_name=$(basename -s ".zip" "${zip_file}")
    version="${zip_name#*-}"
    zip_url="https://download.zeroc.com/${download_path}/${zip_name}.zip"

    checksum=$(shasum -a 256 "${zip_file}" | cut -d ' ' -f 1)
    indent=$(printf "%12s" "") # indentation for the checksum line

    # Replace path: "..." with url/checksum lines for slice2swift
    sed -i '' -e "s|path: \"cpp/bin/slice2swift\.zip\"|url: \"${zip_url}\",\n${indent}checksum: \"${checksum}\"|" Package.swift
done
